...
static void *md_thread(void *arg)
{
    ...
    while (ctx->running) {
        bt_md_tick_t tick;
        if (!BT_SPSC_POP(*ctx->in_queue, &tick)) { __builtin_ia32_pause(); continue; }
        ctx->tick_count++;
    }
...
bt_md_ctx_t *bt_md_create(int tid, int cpu, bt_md_tick_queue_t *in)
{
    ...
    if (!ctx) return NULL;
    ctx->thread_id = tid; ctx->cpu_core = cpu; ctx->in_queue = in; ctx->running = 1;
    return ctx;
}
int  bt_md_start(bt_md_ctx_t *ctx) { return ctx ? pthread_create(&ctx->thread, NULL, md_thread, ctx) : -1; }
void bt_md_stop(bt_md_ctx_t *ctx)  { if (ctx) { ctx->running = 0; pthread_join(ctx->thread, NULL); } }
void bt_md_destroy(bt_md_ctx_t *ctx) { if (ctx) { if (ctx->running) bt_md_stop(ctx); free(ctx); } }
